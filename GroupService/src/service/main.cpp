...
int main(int argc, char **argv)
{
    ...
    // 绑定地址
    std::shared_ptr<GroupService> group_service = std::shared_ptr<GroupService>(new GroupService());
    ...
    // 注册服务
    lightrpc::RpcDispatcher::GetRpcDispatcher()->RegisterServlet("/GroupService", group_service);
    // 构建服务端
    server_->Start();
