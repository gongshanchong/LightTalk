...
class AddGroupImpl : public lightrpc::RpcInterface{
public:
    AddGroupImpl(const google::protobuf::Message* req, google::protobuf::Message* rsp, google::protobuf::RpcController* controller, ::google::protobuf::Closure* done):
    lightrpc::RpcInterface(req, rsp, controller, done){}

    void Run(){
        try {
            // 初始化请求和响应
            auto req = dynamic_cast<const ik_ChatService::AddGroupRequest*>(m_req_base_);
            ...
            if (send(client_fd, send_str.c_str(), send_str.size(), 0) == -1){
                throw "addgroup command send error";
            }
