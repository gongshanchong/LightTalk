...
{
    // 注册连接事件回调函数
    server_.setMessageCallback(bind(&ProxyServer::OnMessage, this, _1, _2, _3));

    // 注册连接事件回调函数
    server_.setConnectionCallback(bind(&ProxyServer::OnConnet, this, _1));
...
// 连接事件的回调函数
void ProxyServer::OnConnet(const muduo::net::TcpConnectionPtr &conn)
{
    if (!conn->connected())
    {
        //  当客户端异常退出，执行此命令
        ProxyService::get_instance().ClientCloseException(conn);
        conn->shutdown();
    }
}
...
void ProxyServer::OnMessage(const muduo::net::TcpConnectionPtr &conn, muduo::net::Buffer *buffer, muduo::Timestamp stamp)
{
    ...
    // 获取对应的处理器并执行
    auto msg_handler = ProxyService::get_instance().GetHandler(request.type());
