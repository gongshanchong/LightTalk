...
class LoginOutImpl : public lightrpc::RpcInterface{
public:
    LoginOutImpl(const google::protobuf::Message* req, google::protobuf::Message* rsp, google::protobuf::RpcController* controller, ::google::protobuf::Closure* done):
    lightrpc::RpcInterface(req, rsp, controller, done){}

    void Run(){
        try {
            // 初始化请求和响应
            auto req = dynamic_cast<const ik_UserService::LoginOutRequest*>(m_req_base_);

            ZKMatser::getInstance()->get_follow();
            ik_UserServer::Request loginout_request;
            loginout_request.set_type("LoginOut");
            loginout_request.set_request(req->SerializeAsString());
            ...
            if (send(client_fd, send_str.c_str(), send_str.size(), 0) == -1){
                throw "loginout command send error";
            }
