...
int main(int argc, char **argv)
{
    ...
    // 绑定地址
    std::shared_ptr<UserService> user_service = std::shared_ptr<UserService>(new UserService());
    ...
    // 注册服务
    lightrpc::RpcDispatcher::GetRpcDispatcher()->RegisterServlet("/UserService", user_service);
    // 构建服务端
    server_->Start();
