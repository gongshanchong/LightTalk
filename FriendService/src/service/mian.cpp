...
int main(int argc, char **argv)
{
    ...
    // 绑定地址
    std::shared_ptr<FriendService> friend_service = std::shared_ptr<FriendService>(new FriendService());
    ...
    // 注册服务
    lightrpc::RpcDispatcher::GetRpcDispatcher()->RegisterServlet("/FriendService", friend_service);
    // 构建服务端
    server_->Start();
